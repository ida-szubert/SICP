...
(define (create-instance maker . args)
  (let* ((instance (make-instance))
         (handler (apply (cadr maker) instance args)))
...
(define (->handler x)
  (cond ((instance? x) (cdr (instance-handler x)))
	     ((handler? x) (cdr x))
...
(define (show-handler proc)
  ...
  (if (handler? proc)
	...
	(let ((methods (environment-lookup (procedure-environment proc)
      ...
      'not-a-handler))
|#
;; These procedures would be difficult to re-write for Racket.
;; The reason is that environment is not a first-class citizen in Racket Scheme, and there's no substitute for environment-bindings
