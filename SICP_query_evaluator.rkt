    ...
    (cond ((assertion-to-be-added? q) (add-rule-or-assertion! (add-assertion-body q))
          ...
          (else (newline)
                ...
                (display-stream (stream-map (lambda (frame)
                                              (instantiate q frame (lambda (v f) (consract-question-mark v))))
                                            (qeval q (singleton-stream '()))))
                (query-driver-loop)))))
                                                

(define (instantiate exp frame unbound-var-handler) ;the last argument is a procedure to be performed when a variable cannot be instantiated
  (define (copy exp)
...
(define (qeval query frame-stream)
  (let ((qproc (get (type query) 'qeval))) ;that's for special forms
    (if qproc
...
(define (execute exp)
  (apply (eval (predicate exp) user-initial-environment) ;only the predicate in an expression that needs evaluating
         (args exp)))

(define (always-true ignore frame-stream) ;passes through all the frames in the input stream
  frame-stream)

(define (find-assertions pattern frame)
  (stream-flatmap (lambda (datum) (check-an-assertion datum pattern frame))
                  (fetch-assertions pattern frame))) ;fetch-assertions in used in order to optimize matching- not all assertions will be inspected
...
(define (apply-rules pattern frame)
  (stream-flatmap (lambda (rule) (apply-a-rule rule pattern frame))
                  (fetch-rules pattern frame))) ;fetch-rules has an analogous role to fetch-assertions

(define (apply-a-rule rule query-pattern query-frame) ;it renames all the variables in the rule (diffrent rules might use the same variable names, we don't want a clash)
...
(define (extend-if-possible var val frame)
  (let ((binding (binding-in-frame var frame)))
    (cond (binding (unify-match (binding-value binding) val frame))
          ...
          ((depends-on? val var frame) 'failed) ;this tests whether expression proposed as the value depends itself on the variable
...
;all assertions of a data base are stored in one stream
;additionally, assertions with the same constant in car are stored in theor own, separate stream, which is indexed by that constant
;rules are stored similarly, usin car of the conclusion
;only that when faced with a pattern with constant car, we need to fetch rules starting with the same car and rules starting with a variable
...
(define (get-indexed-rules pattern)
  (stream-append (get-stream (index-key-of pattern) 'rule-stream)
                 (get-stream '? 'rule-stream))) ;that's the stream of rules starting with a variable
...
(define (map-over-symbols proc exp)
  (cond ((pair? exp) (cons (map-over-symbols proc (car exp))
        ...
        ((symbol? exp) (proc exp))
        (else exp)))
(define (expand-question-marki symbol)
  (let ((chars (symbol->string symbol)))
    (if (string=? (substring chars 0 1) "?")
        (list '?
              (string->symbol (substring chars 1 (string-length chars))))
        symbol)))
;all that is done so that variables in a pattern can be recognized by having ? as car
