...
(define (make-random-web size guarantee-path-to-all-nodes?)
  ...
  (let ((n-total-names (vector-length *all-words*)))
    (when *random-web-debugging* (begin (newline) (display "Choosing node names...")))
    (let ((node-names
           (map (lambda (n)
                  (string->symbol
                   (string-append
	  ...
	  (when *random-web-debugging*
	      (begin (newline) (display (list name (length exits) (length words)))))
          (list name
                (let ((random-exits
      ...
      (when *random-web-debugging* (begin (newline) (display "creating nodes...")))
