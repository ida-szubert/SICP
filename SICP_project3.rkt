  ...
  (let ((entry (find-entry-in-index index key))
    ...
    (if (null? entry)
        ...
        (let ((current-values (get-value entry)))
          (set-cdr! entry (list (add-if-not-present value current-values))))))
  'ok)
...
(define (normal-index web initial-url)
  (define index (make-index))
  (define (inner-index-maker graph start)
    (BFC start
         (lambda (url) (add-document-to-index! index graph url))
         graph))
  (inner-index-maker web initial-url)
  index)

(define normal-web-index (normal-index the-web 'http://sicp.csail.mit.edu/))

(define (get-keys index)
  (map get-key (cdr index)))
...
(define (optimize-index index)
  (let ((keys (get-keys index)))
    (let ((sorted-keys (sort keys alphabetic-order?))
          (vector-length (length keys)))
      (define vector (make-vector vector-length '()))
      (define (inner-optimize key-list count)
        (if (>= count vector-length)
            (list 'index vector)
            (begin (vector-set! vector count (find-entry-in-index index (car key-list)))
                   (inner-optimize (cdr key-list) (+ count 1)))))
      (inner-optimize sorted-keys 0))))

(define optimized-web-index (optimize-index normal-web-index))


(define (vector-get-entries index) (cadr index))
(define (vector-get-keys index)
  (let ((entries (vector-get-entries index)))
    (let ((length (vector-length entries)))
      (define (help keys count)
        (if (>= count length)
            (reverse keys)
            (help (cons (car (vector-ref entries count)) keys)
                  (+ count 1))))
      (help '() 0))))
   

(list-ref (vector-get-keys optimized-web-index) 10)
(list-ref (get-keys normal-web-index) 10)
;(find-entry-in-index optimized-web-index '|23|)
(find-entry-in-index normal-web-index '|23|)
;(find-entry-in-index optimized-web-index 'sense)
(find-entry-in-index normal-web-index 'sense)


;; Problem 9

(define (vector-find-entry-in-index index key)
  (let ((entries (vector-get-entries index)))
      (define (inner-search low high)
        (let ((n (average low high)))
        (let ((nth-key (get-key (vector-ref entries n))))
          (cond ((= low high) (if (equal? nth-key key) (vector-ref entries n) '()))
                ((equal? nth-key key) (vector-ref entries n))
                ((alphabetic-order? key nth-key) (inner-search low n))
                (else (inner-search n high))))))
    (inner-search 0 (vector-length entries))))

(define (average a b) (floor (/ (+ a b) 2)))
(vector-find-entry-in-index optimized-web-index '|23|)
(vector-find-entry-in-index optimized-web-index 'sense)